...
from comunidadeimpressionadora.forms import FormLogin, FormCriarConta, FormEditarPerfil, FormCriarPost, FormContato
from comunidadeimpressionadora.models import Usuario, Post, Curso
...
# esse é o arquivo responsavel por definir as rotas das paginas
...
def login():
    ...
    if form_login.validate_on_submit() and 'botao_submit_login' in request.form:
        ...
        if usuario and bcrypt.check_password_hash(usuario.senha, form_login.senha.data):
            ...
            flash('Bem vindo a Comunidade Impressionadora de Experts da Hashtag Treinamentos!', 'alert-success')
    ...
    if form_criarconta.validate_on_submit() and 'botao_submit_criarconta' in request.form:
        # aqui iremos criptografar a senha para não ficar visivel pra qualquer um no nosso banco de dados
        senha_crypt = bcrypt.generate_password_hash(form_criarconta.senha.data)
        # agora, quando formos chamar a classe do formulário de usuario, iremos gravar a senha cript acima, repare:
...
def editar_perfil():
    ...
    if form.validate_on_submit():
        ...
        # mudar o campo foto_perfil do usuario para o novo nome da imagem que foi carregada no campo de foto
        if form.foto_perfil.data:
            nome_imagem = salvar_imagem(form.foto_perfil.data)
            current_user.foto_perfil = nome_imagem
        current_user.cursos = request.form.getlist('meuscursos')
        database.session.commit()
        flash(f'Perfil atualizado com Sucesso!', 'alert-success')
        return redirect(url_for('perfil'))
    # o elif abaixo carrega automaticamente os campos com os dados do usuario quando clicamos no botao editar
